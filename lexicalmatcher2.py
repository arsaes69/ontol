...
def load_ontology(file_path):
    g = Graph()
    g.parse(file_path, format="xml")  # Cambiar a 'xml' para RDF/XML
    return g
...
def extract_entities(graph):
    ...
    classes = set(graph.subjects(RDF.type, owl_class))
    properties = set(graph.subjects(RDF.type, owl_property))
...
def save_alignments(equivalences, file_path, predicate):
    g = Graph()
    for e1, e2 in equivalences:
        ...
        g.add((ns1[local_name1], predicate, ns2[local_name2]))
...
# Save alignments
save_alignments(class_equivalences, "output/class_alig_cmt-ekaw.ttl", URIRef("http://www.w3.org/2002/07/owl#equivalentClass"))
save_alignments(property_equivalences, "output/property_alig_cmt-ekaw.ttl", URIRef("http://www.w3.org/2002/07/owl#equivalentProperty"))
save_alignments(instance_equivalences, "output/instance_alig_cmt-ekaw.ttl", URIRef("http://www.w3.org/2002/07/owl#sameAs"))
